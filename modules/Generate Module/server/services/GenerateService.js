...
export default class CsvGeneratorService {
	...
	esFetchData = async (_req, indexPatternTitle, body, list_columns_date) => {
		try {
			...
			console.log(
				'list_columns_date.toString() is ',
				list_columns_date.toString()
			);

			let docvalues = [];

			// console.log('docvalues is ', docvalues);
			...
			console.log('docvalues is ', docvalues);
			console.log('newBody  object is ', newBody);
			console.log('body  is ', body);
			console.log('newBody is', JSON.stringify(newBody));
	...
	genereteReport = async (_req, documentId, username, strFilename) => {
		...
		//console.log('filters is ', JSON.parse(filters));
		...
		for (const item of savedSearchInfos._source.references) {
			if (item.name === JSON.parse(filters).indexRefName) {
				...
				//search part
				const operator = this.containsOperator(
					JSON.parse(filters).query.query,
					['AND', 'OR']
				);
				if (JSON.parse(filters).query.query) {
					if (operator === null) {
						...
						if (searchQuery.length === 1) {
							requestBody.must(
								esb
									.multiMatchQuery([], searchQuery[0].trim())
									.type('best_fields')
							);
						...
						switch (operator) {
							...
							case 'AND':
								searchQuery.forEach((query) => {
									const splitSearch = query.split(':');
									requestBody.must(
										esb.matchQuery(splitSearch[0].trim(), splitSearch[1].trim())
									);
								});
								break;
						}
				...
				if (nb_countDiv > 0) {
					for (let i = 0; i < nb_countDiv + 1; i++) {
						const resScroll = await this.esFetchScroll(
							_req,
							resData._scroll_id
						);
				...
				for (const valueRes of arrayHits) {
					for (const data_ of valueRes.hits) {
						...
						//console.log('list_columns_date ', list_columns_date);
						// console.log('data_ is ', data_);
						const fields = data_.fields;
						// console.log('fields is ', data_.fields);
						// console.log('list_columns_date is ', list_columns_date);
						// for(let dateType of list_columns_date){
						//   if(data_._source[dateType]){
						//     console.log('dateType is ', dateType);
						//     console.log('fields is ', fields);
						//     console.log('fields[dateType] is ', fields[dateType]);
						//     console.log('type of fields[dateType] is ', typeof(fields[dateType]));
						//     console.log('fields[dateType] is ', fields[dateType]);
						//     console.log('fields[dateType][0] is ', fields[dateType][0]);
						//     //console.log('fields[dateType] is ', fields[dateType]);
						//     //console.log('moment 0 is ', fields[dateType]);
						//     data_._source[dateType] = moment.utc(fields[dateType][0]).format(EXCEL_FORMAT);
						//     console.log('moment 1 is ', data_._source[dateType]);
						//   }
						// }
						if (fields_exist === true) {
