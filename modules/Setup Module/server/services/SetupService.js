...
export default class SetupService {
	...
	checkIndexExist = async (_req) => {
		try {
			...
			const indexExist = await callWithRequest(_req, 'indices.exists', {
				index: INDEX_NAME,
			});
	...
	//create the index and applies the mappings.
	createReportingIndex = async (_req) => {
		try {
			const { callWithRequest } = this.esDriver.getCluster('data');
			const mappings = await callWithRequest(_req, 'indices.create', {
				index: INDEX_NAME,
			});
	...
	// First check if the index exists. If not create then otherwise do nothing.
	setup = async (_req) => {
		try {
